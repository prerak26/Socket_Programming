...
int main(int argc, char *argv[])
{
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        for( auto it: r_msg[5]){
                            ...
                            int numberoffilesuhave=0;
                            for(auto it3: search_files){
                                if(files_to_be_found[it3]== peer_id){
                                        numberoffilesuhave++;    
                                }
                            }
                            string msg=to_string(numberoffilesuhave)+" ";
                ...
                if (send(it.second.second, (msg.c_str()), msg.length(), 0) == -1){
                    ...
                    if (nbytes <= 0)
                    {
                        ...
                        //del_from_pfds(pfds, i, &fd_count);
                         ...
                         int p = stoi(temp);  
                         cout<<p<<endl;
                        if (send(it.second.second,"OK", 2, 0) == -1){
                        perror("send");
                        }
                        while(p--){
                            ...
                            if (nbytes <= 0)
                            {
                                ...
                                char * buffer;     //buffer to store file contents
                                long size;     //file size
                                ifstream file (argv[2]+temp, ios::in|ios::binary|ios::ate);     //open file in binary mode, get pointer at the end of the file (ios::ate)
                                size = file.tellg();     //retrieve get pointer position
                                file.seekg (0, ios::beg);     //position get pointer at the begining of the file
                                buffer = new char [size];     //initialize the buffer
                                file.read (buffer, size);     //read file to buffer
                                file.close();     //close file
                                sendall(it.second.second,temp, buffer, &size);
    ...
    cout<<"reached last while loop"<<endl;
    for(auto it:search_files_2){
        ...
        for (auto neighbour : n_sockfd){
                ...
                cout<<"msg6#send to neighbour"<<endl;
        }
    }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        for(auto it: r_msg[5]){
                            ...
                            cout<<numberoffilesuhave<<" send"<<endl;
                            char recvbuf[numbytes];
                            int nbytes = recv(pfds[i].fd, recvbuf, sizeof recvbuf, 0);
                            recvbuf[nbytes] = '\0';  
                            if (nbytes <= 0)
                            ...
                            if(it[0]=='S'){
                                ...
                                for(auto it2: n_sockfd){
                                    if(it2.first==stoi(temp)){
                                        ...
                                        // string msg=r_msg;
                                        r_msg_1+="#";
                                        cout<<"sending to 1 msg from 2"<<endl;
                                        cout<<r_msg_1<<endl;
                                        if (send(it2.second.second, (r_msg_1.c_str()), r_msg_1.length(), 0) == -1){
                                            perror("send");
                                        }
                        ...
                        for(auto it: r_msg[7]){
                            ...
                            if(it[0]=='S'){
                                ...
                                // }
                                for(auto file:files){
                                    cout<<file<<" "<<strlen(file)<<" "<<temp<<endl;
                                    if(file==temp2){
                                        cout<<"compdone"<<endl;
                                        client_no=OWN_UNIQUE_ID;
                                        break;
